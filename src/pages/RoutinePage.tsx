...
export function RoutinePage({ currentUser, selected, tasks, onSelect, onAdd }: { currentUser: Person; selected: Person; tasks: RoutineTask[]; onSelect: (p: Person) => void; onAdd: () => void }) {
  ...
  return <main className="page routine-page">
    <header className="home-header"><div className="brand-row"><span className="brand-dot" />Nossa Rotina</div><p className="date"><CalendarDays size={16} />{date}</p><h1>{greeting}, {PERSON_NAMES[currentUser]} <Hand aria-hidden="true" /></h1><p className="subtitle">Um passo de cada vez, no seu ritmo.</p></header>
    <RoutineTabs currentUser={currentUser} selected={selected} onSelect={onSelect} />
    {tasks.length ? <div className="sections">{periods.map(period => <RoutineSection key={period.key} icon={period.icon} title={period.title} tasks={tasks.filter(task => task.period === period.key).sort((a, b) => a.order - b.order)} currentId={currentId} />)}</div> : <section className="empty-state"><Sprout aria-hidden="true" /><h2>Nenhuma atividade cadastrada.</h2><p>Adicione atividades para montar a rotina da {PERSON_NAMES[selected]}.</p><button className="button primary" onClick={onAdd}><Plus size={18} />Adicionar atividade</button></section>}
  </main>;
}
